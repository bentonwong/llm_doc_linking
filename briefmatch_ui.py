...
# def analyze_briefs(moving_json, response_json, top_n, highlight):
#     try:
#         payload = {
...
#         html_output = f"<h3>Top Matches (Brief {data['moving_brief_id']} vs {data['response_brief_id']})</h3><ul>"

#         for link in data["top_links"]:
#             if highlight:
#                 m_content, r_content = highlight_common_terms(link["moving_content"], link["response_content"])
#             else:
#                 m_content, r_content = link["moving_content"], link["response_content"]

#             html_output += f"""
#                 <li>
#                     <strong>🔹 Moving Heading:</strong> {link['moving_heading']}<br>
#                     <div style="margin-left: 1em; color: darkblue;">{m_content}</div><br>
#                     <strong>🔸 Response Heading:</strong> {link['response_heading']}<br>
#                     <div style="margin-left: 1em; color: darkgreen;">{r_content}</div><br>
#                     <strong>🔗 Score:</strong> {link['score']:.3f}
#                 </li><hr>
#             """
#         html_output += "</ul>"
#         return html_output
...
# highlight_checkbox = gr.Checkbox(label="🔦 Highlight shared keywords", value=True)

# with gr.Blocks(title="🧠 Brief Match") as demo:
#     gr.Markdown("### Analyze and compare moving and response briefs. Shared terms will be <mark>highlighted</mark> in the results.")

#     with gr.Row():
#         moving_input.render()
#         response_input.render()

#     top_n_input.render()
#     highlight_checkbox.render()
#     submit_button = gr.Button("🔍 Analyze")
#     output_html = gr.HTML(label="🔗 Highlighted Top Matches")

#     submit_button.click(
#         fn=analyze_briefs,
#         inputs=[moving_input, response_input, top_n_input, highlight_checkbox],
#         outputs=output_html
#     )

#     demo.load(get_random_pair, inputs=[], outputs=[moving_input, response_input, top_n_input, highlight_checkbox])
...
def analyze_briefs(moving_json, response_json, top_n):
    try:
        ...
        return plain_html, highlighted_html, gr.update(visible=True), f"✅ **Completed in {elapsed:.2f} seconds**"

    except Exception as e:
        error_html = f"<pre>❌ Error: {str(e)}</pre>"
        return error_html, error_html, gr.update(visible=False), f"❌ Error: {str(e)}"
...
with gr.Blocks(title="🧠 Brief Match") as demo:
    ...
    top_n_input = gr.Number(label="Top N Matches", value=1)
    highlight_checkbox = gr.Checkbox(label="🔦 Highlight shared keywords", value=False, visible=False)  # <--- move this here
    submit_button = gr.Button("🔍 Analyze")
    status_output = gr.Markdown("")  # initially empty
    output_html = gr.HTML()
    ...
    # Submit and store both versions
    # submit_button.click(
    #     fn=analyze_briefs,
    #     inputs=[moving_input, response_input, top_n_input],
    #     outputs=[output_html, highlighted_html_state, highlight_checkbox],
    # ).then(
    #     fn=lambda h: h,  # set plain version as initial state
    #     inputs=output_html,
    ...
    submit_button.click(
        ...
        fn=analyze_briefs,
        inputs=[moving_input, response_input, top_n_input],
        outputs=[output_html, highlighted_html_state, highlight_checkbox, status_output],
    ).then(
        fn=lambda h: h,
        inputs=output_html,
        outputs=plain_html_state
